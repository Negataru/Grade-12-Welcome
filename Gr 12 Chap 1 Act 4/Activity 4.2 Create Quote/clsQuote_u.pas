...
begin
  Result := fLength * fWidth * fRatePerSqrM;
end;

function TQuoteCalculator.CalculateMaterialCost : real;
begin
   Result := CalculateNumBoxes * fPricePerBox;
end;

function TQuoteCalculator.CalculateNumBoxes : integer;
begin
   Result := Round((fLength * fWidth) / fCoverAreaPerBox);
end;

function TQuoteCalculator.CalculateTotalCost: real;
begin
    Result := CalculateMaterialCost + CalculateLabourCost;
end;
